...
export function WorkExperienceForm({
  ...
  const updateWorkExperience = (
    ...
    onChange(
      data.map((work) => (work.id === id ? { ...work, [field]: value } : work))
    );
  };
            ...
            >
              <div className="flex justify-between items-start">
                <h4 className="font-medium bg-violet-200 p-1 rounded-lg">
                  Experience {index + 1}: {work.position || "New Position"}
                </h4>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <div className="space-y-2">
                ...
                {work.description.map((desc, index) => (
                  <div key={index} className="flex gap-2">
                    <Textarea
                      value={desc}
                      onChange={(e) =>
                        updateDescription(work.id, index, e.target.value)
                      }
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => addDescriptionPoint(work.id)}
                  className="w-full"
                >
                  <Plus className="h-4 w-4 mr-2" />
